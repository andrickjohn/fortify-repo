...
const PDFIngestion = ({
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    try {
      ...
      // 1. Check for duplicates (More robust check)
      // Sometimes the DB has "PO T80P0540 - SINGLEWIRE SOFTWARE LLC" but the file is "T80P0540 - SINGLEWIRE"
      // Extract potential contract number (e.g. T80P0540, W80P0017)
      const numberMatch = contractName.match(/([A-Z0-9]{8})/);
      const possibleNumber = numberMatch ? numberMatch[1] : null;
      ...
      if (possibleNumber) {
        query = query.or(
          `contract_name.ilike.%${contractName}%,contract_name.ilike.%${possibleNumber}%,contract_number.eq.${possibleNumber}`,
        );
      } else {
        query = query.ilike("contract_name", `%${contractName}%`);
      }

      const { data: existingRecords } = await query.limit(1);

      if (existingRecords && existingRecords.length > 0) {
        setDuplicateUploadData({
          newFile: file,
          existingContract: existingRecords[0],
        });
  ...
  const handleReplace = async () => {
    if (!duplicateUploadData || !districtId) return;

    const { newFile, existingContract } = duplicateUploadData;

    try {
      // 1. Upload new file to Storage (overwrite or unique name)
      const filePath = `uploads/${Date.now()}_${newFile.name.replace(/\\s+/g, "_")}`;
      const { data: uploadData, error: uploadError } = await supabase.storage
        .from("contracts")
      ...
      // 2. Parse PDF for new Purpose
      let purpose = null;
      const text = await extractTextFromPDF(newFile);
      purpose = extractPurpose(text);

      // 3. Update existing contract record
      const { error: updateError } = await supabase
        .from("contracts")
        .update({
          document_url: publicUrl,
          description_of_purpose: purpose,
  ...
  return (
    ...
    >
      ...
      <DuplicateResolutionModal
        isOpen={!!duplicateUploadData}
        newFile={duplicateUploadData?.newFile || null}
        existingContract={duplicateUploadData?.existingContract || null}
        onSkip={() => setDuplicateUploadData(null)}
...
function ContractsPageContent() {
  ...
  const groupedContracts = React.useMemo(() => {
    ...
    contracts.forEach((c) => {
      const vName = Array.isArray(c.vendors)
        ? c.vendors[0]?.vendor_name
        : c.vendors?.vendor_name || "Unknown Vendor";
    ...
    return Object.entries(groups)
      .map(([vendor, items]) => ({
        ...
        totalValue: items.reduce(
          (sum, item) => sum + (item.annual_value || 0),
          0,
        ),
        category: Array.isArray(items[0].vendors)
          ? items[0].vendors[0]?.category
          : items[0].vendors?.category,
      }))
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-100">
                  <th className="pl-6 pr-6 py-4 text-[10px] uppercase tracking-widest font-black text-slate-400">
                    Contract / Vendor
                  </th>
                  ...
                  <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-slate-400 text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                      ...
                      <tr className="group hover:bg-blue-50/10 transition-colors">
                        <td className="pl-6 pr-6 py-4">
                          <div className="flex items-center space-x-2">
                            ...
                            <div className="flex items-center space-x-3">
                              <div
                                className={`p-2 bg-white border border-slate-100 rounded-lg ${hasChildren ? "border-dashed" : ""} ${!hasChildren ? "ml-2" : ""}`}
                              >
                                {hasChildren ? (
                                ...
                                <div className="font-bold text-slate-900 text-sm">
                                  ...
                                  {hasChildren && (
                                    <span className="text-slate-400 text-xs font-normal ml-2">
                                      ({group.items.length} Contracts)
                                    </span>
                                  )}
                                </div>
                                {!hasChildren && (
                                  <div className="text-[10px] text-slate-400 font-medium font-mono">
                                    {mainContract.contract_number ||
                                      mainContract.contract_name}
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-1.5 w-max">
                              ...
                              >
                                ...
                                {mainContract.ai_status === "in_progress" &&
                                  "Analyzing..."}
                                {mainContract.ai_status === "completed" &&
                                  "View Recommendation"}
                                {mainContract.ai_status === "approved" &&
                                  "Approved"}
                                {mainContract.ai_status ===
                                  "negotiation_started" &&
                                  "Negotiation Created"}
                          ...
                          >
                            ...
                            <td className="px-6 py-3">
                              <div className="flex flex-col gap-1.5 w-max">
                                <button
                                  ...
                                  disabled={
                                    !isAISetup &&
                                    contract.ai_status === "not_started"
                                  }
                                  className={`
                            ...
                            <td className="px-6 py-3">
                              <div className="flex flex-col">
                                ...
                                {(!contract.annual_value ||
                                  Number(contract.annual_value) === 0) &&
                                  contract.description_of_purpose && (
